...
def create_enhanced_cnn_lstm_model():
    ...
    lstm1 = LSTM(128, return_sequences=True,
                ...
                recurrent_regularizer=tf.keras.regularizers.l2(1e-4))(combined)
    dropout1 = Dropout(0.5)(lstm1)          # increase from 0.3
    lstm2 = LSTM(64,                         # reduce from 128
                kernel_regularizer=tf.keras.regularizers.l2(1e-4))(dropout1)
    dropout2 = Dropout(0.5)(lstm2)
    dense1 = Dense(32, activation='relu',   # reduce from 64
                kernel_regularizer=tf.keras.regularizers.l2(1e-4))(dropout2)
    ...
    lr_schedule = CosineDecay(
        initial_learning_rate=1e-4,   # 10x lower than current
        decay_steps=787 // BATCH_SIZE * 50,  # decay over ~50 epochs
        alpha=1e-6
    )
...
def video_level_split(y, sequence_video_ids, val_size=0.2, random_state=42):
    ...
    for lbl, sources in class_sources.items():
        ...
        rng.shuffle(sources)
        n_val = 1 if len(sources) <= 3 else max(1, math.ceil(len(sources) * val_size))
...
def train_enhanced_model_with_visualizations(X_flow, X_scalar, y, original_frames=None, sequence_video_ids=None):
    ...
    # The 4 scalar features (flow acceleration, divergence, scene changes,
    # motion entropy) have very different scales compared to the optical flow
    # input which is already in [-1, 1].  Without normalisation the LSTM
    # gradients are destabilised and training diverges early.
    #
    # IMPORTANT: We fit the scaler ONLY on the training set, then apply the
    # same transform to the validation set and save it for inference time.
    # -------------------------------------------------------------------------
    scalar_shape_train = X_scalar_train.shape   # (N_train, 16, 4)
    scalar_shape_val   = X_scalar_val.shape     # (N_val,   16, 4)

    scaler = StandardScaler()
    # Flatten to 2-D so sklearn can process it: (N*16, 4)
    X_scalar_train_2d = X_scalar_train.reshape(-1, scalar_shape_train[-1])
    X_scalar_val_2d   = X_scalar_val.reshape(-1,   scalar_shape_val[-1])

    # fit on training data ONLY, then transform both splits
    X_scalar_train_2d = scaler.fit_transform(X_scalar_train_2d)
    X_scalar_val_2d   = scaler.transform(X_scalar_val_2d)

    # Reshape back to original 3-D shape
    X_scalar_train = X_scalar_train_2d.reshape(scalar_shape_train)
    X_scalar_val   = X_scalar_val_2d.reshape(scalar_shape_val)

    # Save scaler so predict_with_enhanced_model can use the same transform
    joblib.dump(scaler, 'scalar_feature_scaler.pkl')
    print("Scalar features normalised with StandardScaler. Scaler saved to scalar_feature_scaler.pkl")
    print("X_scalar_train stats after scaling:", X_scalar_train.min(), X_scalar_train.max())
    ...
    early_stopping = EarlyStopping(
        monitor='val_accuracy',   # match the checkpoint monitor
        patience=15,
    ...
    history = model.fit(
        ...
        callbacks=[checkpoint, early_stopping],   # <-- comma was missing here
...
def predict_with_enhanced_model(model, video_path, temp_dir="temp_optical_flow", timeout_seconds=300):
    ...
    # -------------------------------------------------------------------------
    # APPLY SCALAR SCALER (must match what was used during training)
    # -------------------------------------------------------------------------
    scaler_path = 'scalar_feature_scaler.pkl'
    if os.path.exists(scaler_path):
        import joblib
        inference_scaler = joblib.load(scaler_path)
        scalar_shape_pred = X_scalar_pred.shape          # (N, 16, 4)
        X_scalar_pred_2d  = X_scalar_pred.reshape(-1, scalar_shape_pred[-1])
        X_scalar_pred_2d  = inference_scaler.transform(X_scalar_pred_2d)
        X_scalar_pred     = X_scalar_pred_2d.reshape(scalar_shape_pred)
        print("Scalar features normalised using saved scaler.")
    else:
        print("WARNING: scalar_feature_scaler.pkl not found. "
              "Predictions may be inaccurate. Train the model first to generate the scaler.")
...
def main():
    ...
    parser.add_argument('--video-path', type=str,
                        default=r"C:\MMM\stamp_detect\Untitled video - Made with Clipchamp (2).mp4", #change this
    ...
    # Set the data path
    data_path = r"C:\MMM\stamp_detect\Stampede_detection_dataset\Stampede_detection_dataset"  #change this
